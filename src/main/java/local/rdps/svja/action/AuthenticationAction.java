...
/**
 * <p>
 * This class contains the authentication methods that should be used by Struts.
 * </p>
 *
 * @author DaRon
 * @since 1.0
 */
public class AuthenticationAction extends RestAction {
	...
	private String authenticateViaLocalDatabase()
		...
		AuthenticationAction.logger.info("User is authenticated as {} with a User-Agent of {}", user.getUsername(),
	...
	private UserVo ensureAccountIsReady()
		...
		return UserManagementBloGateway.verifyUserLogon(user);
	}
	...
	private Optional<String> rekeyTheUser() throws ApplicationException {
		...
		if (ValidationUtils.isEmpty(oldSession)) {
			...
			if (ValidationUtils.not(Objects.equals(oldSession.getId(), newSessionId))) {
				...
				if (AuthenticationAction.logger.isDebugEnabled()) {
					AuthenticationAction.logger.debug(
		...
		} else if (AuthenticationAction.logger.isDebugEnabled()) {
			AuthenticationAction.logger.debug("We couldn't find a session ID to port everything over");
		}
		// No new sessionId
